 ...
 * Функция шифрования
 */
pub fn encode(plain_text: &str, key: &str) -> String {
    if key.is_empty() || plain_text.is_empty() {
        return String::new();
    }
    ...
    plain_text.chars().map(|c| {
        if c.is_ascii_alphabetic() {
            ...
            let new_char = latin_alphabet[(pos + shift) % latin_len];
            ...
            if let Some(pos) = cyr_alphabet.iter().position(|&x| x == lower) {
                ...
                let new_char = cyr_alphabet[(pos + shift) % cyr_len];
                if c.is_uppercase() {
 ...
 */
pub fn decode(plain_text: &str, key: &str) -> String {
    if key.is_empty() || plain_text.is_empty() {
        return String::new();
    }
    ...
    plain_text.chars().map(|c| {
        if c.is_ascii_alphabetic() {
            ...
            let shift = latin_shifts[latin_idx % latin_shifts.len()];
            latin_idx = latin_idx.wrapping_add(1);
            let new_char = latin_alphabet[(pos + latin_len - (shift % latin_len)) % latin_len];
            if c.is_ascii_uppercase() {
                new_char.to_ascii_lowercase()
            } else {
                new_char
            ...
            if let Some(pos) = cyr_alphabet.iter().position(|&x| x == lower) {
                let shift = cyr_shifts[cyr_idx % cyr_shifts.len()];
                cyr_idx = cyr_idx.wrapping_add(1);
                let new_char = cyr_alphabet[(pos + cyr_len - (shift % cyr_len)) % cyr_len];
                if c.is_uppercase() {
                    new_char.to_lowercase().next().unwrap_or(new_char)
                } else {
                    new_char
